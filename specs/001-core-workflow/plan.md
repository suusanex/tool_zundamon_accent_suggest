...
#### A-1) 辞書候補抽出の品質/互換性（2026-02-04 追記）

運用上、「辞書候補抽出で英単語（例: AI）のみが返り、日本語の候補が返らない」事象が発生した。
本件はエンコード起因よりも、プロンプト契約の曖昧さ・実装のJSON契約不一致・構造化出力未強制による不安定さが主因になりやすい。

本機能の期待動作（ユーザー合意）:
- 対象: 日本語台本から、固有名詞・専門用語・特殊読みが必要な語を抽出（一般語は原則除外）
- 出力: 読み（カタカナ）とアクセント核位置を含む、機械可読なJSON
